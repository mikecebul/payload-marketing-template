# PAYLOAD SOHO MARKETING TEMPLATE
...
I'm using Turso which is really the icing on the cake for my dream stack for small websites like this. In Development, just make sure you have the env var LOCAL_DATABASE_URL and it will save a local sqlite db. Super cool to not need to use docker to setup a postgres instance. DB push is automatic in dev, but before pushing changes to github, make sure you `payload migrate:create` to create a new migration file. The SQLite payload adapter is in beta and using an older version from drizzle, so it's not smoothe, but it works. Only real big issue is I didn't realize I had to do so much work just to update a foreign key. This is something I'll need to figure out how to do properly and document it in here. Anyone want to take a crack at a PR?

## Deployment

I'm using Vercel because I'd rather not maintain a Digital Ocean Droplet anymore. The migrate script is in build script already. If you want to build in dev, `pnpm build:dev`.
